...
class Parameterization():
    ...
    def fit(self, x):
        ...
        self.freq_scale = self.freq_scale * 0.75
        self.amp_scale = self.amp_scale * 0.75
    ...
    def transform(self, x):
        ...
        elec = np.vectorize(self.elec2idx.get)(x['elec'])
...
def train_test_split(configs, holdout_frequencies, holdout_amplitudes):
    ...
    test_configs = []
    # for conf in configs:
    #     if conf['ees']['elec'] != 1:  #TODO: dirty fix only for exp20210604  
    ...
    train_data = {
        'ees': {
            ...
            'elec': np.array([conf['ees']['elec'] for conf in train_configs], dtype=np.int32),
        },
    ...
    test_data = {
        'ees': {
            ...
            'elec': np.array([conf['ees']['elec'] for conf in test_configs], dtype=np.int32),
        },
...
class SheepDataModule():
    ...
    def prepare_data(self):
        save_path = os.path.join(self.data_dir, 'data.npy')
        if os.path.isfile(save_path):
            ...
            def visualize_dataset(file, data):
                ...
                for c in np.unique(data['config']):
                    ...
                    elec = data['ees']['elec'][data['config'] == c][0]
            ...
            visualize_dataset(os.path.join(self.data_dir, 'train_dataset.png'), train_data)
            visualize_dataset(os.path.join(self.data_dir, 'test_dataset.png'), test_data)
    ...
    def setup(self, side='both'):
        ...
        data = np.load(os.path.join(self.data_dir, 'data.npy'), allow_pickle=True).item()
